...
router.post(
  ...
  async (req, res) => { //get email and password from req.body and verify user
    ...
    try { // If no error exists in validation , find user's document
      ...
      console.log("err log in route: POST api/auth for Login in user: ",
        err.MongooseServerSelectionError);

      if (err.MongooseServerSelectionError)
        res.status(500).send({ error_message: "Couldn't establish connection to Database. Please check your connnection." });
